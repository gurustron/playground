...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = b.legalNeighbors
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val xx = newFound.map{ case (block, _) => block.isLegal}.toList
    newFound.lazyAppendedAll(from(newFound, explored ++ newFound.map{ case (block, _) => block}))
  }
