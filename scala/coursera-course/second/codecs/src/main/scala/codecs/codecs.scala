...
trait EncoderInstances {
  ...
  implicit val boolEncoder: Encoder[Boolean] = Encoder.fromFunction(Json.Bool)
...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] = Decoder.fromPartialFunction{
    case Json.Num(n) if n.isValidInt => n.toInt
  }

  /** A decoder for `String` values */
  // TODO Define an implicit value of type `Decoder[String]`
  implicit val stringDecoder: Decoder[String] = Decoder.fromPartialFunction{
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(items) =>
        ...
        if(decoded.forall(o => o != None)) Some(decoded.flatten) else None
      case _ => None
    }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =  Decoder.field[String]("name")
    .zip[Int](Decoder.field("age"))
    .transform{ case (str, i) => Person(str, i)}
