...
(Vector i j k) `vplus` (Vector a b c) = Vector (a+i) (b+j) (c+k)
...
lockerLookUp lockerNum map = case Map.lookup lockerNum map of
    Nothing -> Left $ show lockerNum ++ " was not found!"
    Just (st, code) -> if st /= Taken 
        then Right code
        else Left  $ show lockerNum ++ " is taken!" 
...
yesnoIf yyy val noval = if yesno yyy
    then val
    else noval
